...
function hook_listing_templatename($c){
	...
	if($GLOBALS['CORE_THEME']['customlisting_enable'] == 1){
	return "listing";
	}
...
function handle_home_template_object1(){ global $OBJECTS;
    ...
    if(isset($GLOBALS['CORE_THEME']['homepage']) && isset($GLOBALS['CORE_THEME']['homepage']['widgetblock1']) && strlen($GLOBALS['CORE_THEME']['homepage']['widgetblock1']) > 1 && $GLOBALS['CORE_THEME']['homeeditor'] == 1){
...
function gallerypage_results_before(){ global $CORE, $category, $canShowExtras;
	...
	if($GLOBALS['CORE_THEME']['subcategories'] == '1' && $canShowExtras && !isset($_GET['s']) ){ 
		...
		if ($term->parent == 0) {		 
			...
			echo '<div id="wlt_core_subcategories"><ul class="list-inline">'.$cats.'</ul><div class="clearfix"></div></div><hr />';
			}
		}	
	}
}
