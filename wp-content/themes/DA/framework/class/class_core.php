...
class white_label_themes { 
	...
	function constants(){ 	
		...
		// THEME VERSION 
		define("THEME_VERSION", "8.4");		
		// RELEASE DATE
		define("THEME_VERSION_DATE", "10th Aug, 2015");		
		// THEME INSTALL LINK
	...
	function taxonomies(){
		...
		if(is_array($GLOBALS['CORE_THEME'])){	
			...
			if($GLOBALS['CORE_THEME']['sellspace_enable'] == 1){ $canShowAds = true; }else{  $canShowAds = false; }
			 // MESSAGES POST TYPE
			...
			'hierarchical' => true,	
			  ...
			  'show_ui' => $canShowAds,
			  'exclude_from_search' => true,
			  'menu_icon' 	=> get_template_directory_uri()."/framework/admin/img/0/14.png",
			...
			'hierarchical' => true,	
			  ...
			  'query_var' => true,
			  'show_ui' => $canShowAds,
			  'menu_icon' 	=> get_template_directory_uri()."/framework/admin/img/0/15.png",
			  'exclude_from_search' => true,
			...
			if($GLOBALS['CORE_THEME']['feedback_enable'] == 1){ $canShowAds = true; }else{  $canShowAds = false; }
			...
			'hierarchical' => true,	
			  ...
			  'query_var' => true,
			  'show_ui' => $canShowAds,
			  'menu_icon' 	=> get_template_directory_uri()."/framework/admin/img/0/16.png",
			  'exclude_from_search' => true,
		...
		if(isset($_GET['zipcode']) && strlen($_GET['zipcode']) > 3 ){		
			...
			if($range > 0){
				...
				if(is_numeric($longitude) && is_numeric($latitude)){				
					...
					OR wlt2.meta_key =  'map-zip' AND wlt2.meta_value	= '".trim(chunk_split(stripslashes(urlencode($_GET['zipcode'])), 3, ' '))."' ) ";			 
				...
				$q .= "AND (wlt1.meta_key = 'map-zip' AND wlt1.meta_value = ('".strip_tags(strip_tags($_GET['zipcode']))."') OR wlt1.meta_key = 'map-zip' AND wlt1.meta_value = ('".trim(chunk_split(stripslashes(urlencode($_GET['zipcode'])), 4, ' '))."')	) ";
			}
...
function CUSTOMHEADER(){ global $wpdb, $pagenow; $STRING = ""; 
	...
	if(!defined('WLT_CART') && isset($GLOBALS['flag-single']) ){
	...
	// LOAD IN FRAMEWORK CSS FILES
	if(!defined('DISABLE_CORE_CSS')  && isset($GLOBALS['flag-home']) && $GLOBALS['CORE_THEME']['homeeditor'] == 1){
	...
	// DEMO MODE
	if(defined('WLT_DEMOMODE') && !defined('IS_MOBILEVIEW') ){
...
function CUSTOMFOOTER(){
	...
	if(defined('WLT_CART') || ( strpos($post->post_content, "[video ") !== false || strpos($post->post_content, "[audio ") !== false ) ){
				...
				echo hook_single_javascript("<script> jQuery(document).ready(function(){ jQuery('video,audio').mediaelementplayer({ 
				
				   success: function (mediaElement, domObject) { 
         
        // add event listener
        mediaElement.addEventListener('timeupdate', function(e) { jQuery('.videotime".$post->ID."').val(mediaElement.currentTime);
		...
		jQuery('.videotime".$post->ID."').trigger('change');
		  ...
		  mediaElement.addEventListener('ended', function(e) { 
							...
							if(typeof jQuery('.videonextup".$post->ID."') != 'undefined'){					
								setTimeout(function () {
								window.location.href = jQuery('.videonextup".$post->ID."').val();
								}, 500);
							}
...
if($GLOBALS['CORE_THEME']['sellspace_enable'] == 1){
...
foreach($spa as $key => $sp){
	if($sp['p'] == $location && $sellspacedata[$key] == '1'){
		...
		WHERE ".$wpdb->posts.".post_title LIKE ('%".$key."%') 
		AND ".$wpdb->postmeta.".meta_key = 'bannerid'  
		AND ".$wpdb->postmeta.".meta_value != '' 
		AND ".$wpdb->posts.".post_type = 'wlt_campaign'
		AND ".$wpdb->posts.".post_status = 'publish' 
		ORDER BY rand() LIMIT ".$spa[$key]['max'];

		$cc = $wpdb->get_results($SQL, OBJECT); 
 
		if(!empty($cc)){
		
			foreach($cc as $thisone){
			
			// CHECK IF ITS EXPIRED
			$listing_expiry_date = get_post_meta($thisone->ID, 'listing_expiry_date', true);
			if($listing_expiry_date == "" || ( strtotime($listing_expiry_date) < strtotime(current_time( 'mysql' )) )){ 			
				$my_post['ID'] 			= $thisone->ID;
			...
			// GET BANNER DATA
			$bannerID = get_post_meta($thisone->ID, 'bannerid', true);
			 
			if($bannerID != ""){
			
				$bannerIMG = get_post_meta($bannerID, 'img', true);	
				if($bannerIMG == ""){ continue; }	  
				
				// OUTPUT 
				$OUTINNER .= "<a href='".home_url()."/out/".$thisone->ID."/url/' rel='nofollow' target='_blank'><img src='".$bannerIMG."' class='sellspace_banner' alt='".$key."'></a>";
				
				// UPDATE CAMPAIGN IMPRESSIONS
				update_post_meta($thisone->ID,'impressions',get_post_meta($thisone->ID,'impressions',true)+1);
				
				// UPDATE IMPRESSIONS
				$data = get_post_meta($thisone->ID,'impressions_array',true);
				...
				// UPDATE
				update_post_meta($thisone->ID,'impressions_array',$data);
...
if(defined('WLT_DEMOMODE') && $post->post_author == 1){
	$img = array(); $i=0;	 
	while($i < 9){
		$img[$i]['src'] 		= "http://placehold.it/820x550&amp;text=Image+Here";			
		$img[$i]['thumbnail'] 	= "http://placehold.it/820x550&amp;text=Image+Here";
		$img[$i]['name'] 		= "Example Image";
		$img[$i]['id'] 			= "pimg".($i+1);
		$i++;
	}
	$user_attachments = $img;
...
if(is_array($user_attachments) && count($user_attachments) > 0 && $user_attachments[0]['src'] != "" && $user_attachments[1]['src'] != "" ){ 
	...
	if(count($user_attachments) > 1 && isset($user_attachments[1]['thumbnail'])){
		...
		$STRING .= '<script>jQuery(window).load(function(){jQuery("a[data-gal^=\'prettyPhoto\']").prettyPhoto({animation_speed: "normal",	autoplay_slideshow: true,	slideshow: 3000	});});</script>';
	}
...
function FALLBACK_IMAGE($postID=""){
	...
	if(isset($GLOBALS['CORE_THEME']['fallback_image']) && strlen($GLOBALS['CORE_THEME']['fallback_image']) > 5){
		...
		if(defined('WLT_DEMOMODE') && $post->post_author == 1 && $img != ""){
		 
			if(!isset($GLOBALS['imagecount1'])){ $GLOBALS['imagecount1'] = 1; }else{ $GLOBALS['imagecount1']++; }
			$img = str_replace("wlt_thumbnail","wlt_thumbnail pimg".$GLOBALS['imagecount1']."", $img);
		}
...
<div id="textarea_feedback" style="font-size: 12px;  color: #999;  padding-top: 5px;"> <i class="glyphicon glyphicon-tasks" ></i> <?php echo $CORE->_e(array('add','95')); ?>   <span></span> </div>
...
function CSS($tag,$return=false){
	...
	$STRING .= " container-fluid";	
		...
		$STRING = "col-md-4 col-sm-4";
			...
			}elseif($GLOBALS['CORE_THEME']['layout_columns']['3columns'] == "1"){		
			$STRING = "col-md-2 col-sm-2";
			}else{
			$GLOBALS['3COLUMNS_FLAG'] = true;
			$STRING = "col-md-3 col-sm-3";
...
function Logo($return=""){ 
		...
		if(strpos($logo, "http") !== false ){ 
			...
			$logoTXT .= "<div class='logowrapper'><span class='main'>".$logo_text1."</span> <span class='submain'>".$logo_text2."</span></div>";
...
function SIDEBARCONTENT($type, $num = 10, $args = array()){ global $wpdb; $SavedContent = ""; $extra = "";
...
switch($type){
	...
	case "blog": {
	// BLOG POSTS
	$SQL = "SELECT * FROM ".$wpdb->prefix."posts WHERE post_type='post' and post_status='publish' ORDER BY ID DESC LIMIT ".$num;		
	} break;
	default: { 
	
	// LISTINGS
	$SQL = "SELECT * FROM ".$wpdb->prefix."posts WHERE post_type='".THEME_TAXONOMY."_type' and post_status='publish' ORDER BY ID DESC LIMIT ".$num;	
	...
	if(isset($args['type'])){
		switch($args['type']){
			...
			$SQL = "SELECT * FROM ".$wpdb->posts." 
				INNER JOIN ".$wpdb->postmeta." ON ( ".$wpdb->postmeta.".post_id = ".$wpdb->posts.".ID )
				WHERE ".$wpdb->postmeta.".meta_key = 'featured' AND ".$wpdb->postmeta.".meta_value = 'yes' AND ".$wpdb->posts.".post_type = '".THEME_TAXONOMY."_type' LIMIT ".$num;	
			
			} break;
			case "random": {	
					
				$SQL = "SELECT * FROM ".$wpdb->prefix."posts WHERE post_type='".THEME_TAXONOMY."_type' and post_status='publish' ORDER BY RAND() LIMIT ".$num;				
...
if(!empty($result)){
	...
	// DESC
	$DESC = substr(strip_tags($data->post_content),0,80);
	// RATING
	...
	switch($type){
		...
		// IMAGE
		$IMAGE =  get_avatar( $data->user_id, 100 );
		
		// DESC
		$DESC = substr(strip_tags($data->comment_content),0,80); 
	 
		
		?>
        <a href="<?php echo $LINK; ?>">
            <div class="row clearfix commentbit">
            <div class="col-md-3 hidden-sm hidden-xs"><?php echo $IMAGE; ?></div>
            <div class="col-md-9 col-sm-12">
            <?php echo $TITLE; ?>
            <div class="clearfix"></div>
            <div class="quote"><?php echo $DESC; ?></div>
		...
		// CAT
		if(defined('WLT_COUPON')){ $ct = "store"; }else{ $ct = THEME_TAXONOMY; }
		$l = get_the_term_list( $data->ID, $ct, "", ', ', '' );
		$links = explode(",", $l); $string = ""; $lc = 0;
		if(is_array($links)){
			foreach($links as $ll){
			if($lc == 1){ continue; }	
				$string .= ''.$ll;	
        ...
        <div class="col-md-3 hidden-sm hidden-xs nopadding"><?php echo $IMAGE; ?></div>
        <div class="col-md-9 col-sm-12">
        <a href="<?php echo $LINK; ?>">
        	<div class="title"><span><?php echo $TITLE; ?></span></div>
        </a>
        
        <div class="desc"><?php echo $CAT; ?>   </div>
            
        <div class="rating"><?php echo do_shortcode('[RATING style=1 readonly=1 small=1 id='.$data->ID.']'); ?></div>
...
function SELLSPACE($type=1, $userid="", $size=""){ global $wpdb;
	...
	case "3": {  // return user banners
		...
		$SQL = "SELECT ".$wpdb->posts.".* FROM ".$wpdb->posts."
				...
				WHERE ".$wpdb->posts.".post_status= 'publish' LIMIT 0,500";	
...
// BUILD EDIT LINK // DELETE LINK  - this part is a hack for the system to display the correct link - ##### REG #####
if(substr($GLOBALS['CORE_THEME']['links']['add'],-1) != "/"){ 
	//$editlink = $GLOBALS['CORE_THEME']['links']['add']."?eid=".$post->ID;
	$editlink = "/mentoring/index.php/add-profile/"."?eid=".$post->ID;
	$deletelink ="/mentoring/index.php/my-account/"."?did=".$post->ID;
}else{
	//$editlink = $GLOBALS['CORE_THEME']['links']['add']."?eid=".$post->ID;
	$editlink = "/mentoring/index.php/add-profile/"."?eid=".$post->ID;
	$deletelink ="/mentoring/index.php/my-account/"."?did=".$post->ID;
...
function PAYMENTS($amount, $orderID, $description, $postid, $subscription = true  ){ global $userdata, $post, $CORE; $STRING ="";  $packagefields = get_option("packagefields");
	...
	if(is_array($gatway) ){
		...
		if(isset($packagefields[$selected_packageID]['name']) && !defined('WLT_AUCTION') ){
...
<div id="wlt_google_map_wrapper" style="display:none;">
	...
	<div id="wlt_map_toolbox" class="ui-widget-content"> 
...
function wlt_googlemap_data($output = false){  global $post, $query, $wpdb, $CORE;  $g=0; $HasIconArray = array();  $json = array();

	if(isset($_GET['s']) ){
	 
		$gg = explode("LIMIT ",$GLOBALS['wp_query']->request);
		$rd = $wpdb->get_results($gg[0]." LIMIT 300" , OBJECT); 
	...
	if( !empty($rd) ) {	
		...
		foreach($rd as $post){	
			...
			// GET IMAGE
			$image = $this->GETIMAGE($post->ID, false, array('pathonly' => true) );
			...
			"long" 	=> $long, 
			"url"  	=> $permalink,
			"title" => strip_tags(str_replace("'","",$post->post_title)),
			"desc" 	=> strip_tags(str_replace("'","",substr($post->post_content,0,110))),
			"img" 	=> $image,
			"catid" => $catID,	
			"catname" => $catName,	 
			);		 
...
*/
function __MEDIA_GET($postID, $type = 'all'){ global $post, $CORE; $meida_array = array();
