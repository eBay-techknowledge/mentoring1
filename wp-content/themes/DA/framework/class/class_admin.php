...
class wlt_admin { 
  	...
  	function __construct(){ global $pagenow, $OBJECTS, $CORE;
	...
	// CHILD THEMES PAGE LINK
	if(isset($_GET['page']) && $_GET['page'] == "extrathemes"){
	...
	if(isset($_POST['core_system_reset']) && $_POST['core_system_reset'] == "new"){
		...
		if(isset($_POST['submitted']) && $_POST['submitted'] == "yes" && !defined('WLT_DEMOMODE') ){
			...
			// MAMAGEMENT FOR BANNERS
			add_filter('manage_wlt_banner_posts_columns', '_admin_remove_columns');
			add_action( 'manage_wlt_banner_posts_custom_column',  '_admin_custom_column' , 10, 3  );	
			add_filter( 'manage_wlt_banner_posts_columns',  '_admin_custom_columns'  );	
		
			// MAMAGEMENT FOR CAMPAIGN
			add_filter( 'manage_edit-wlt_campaign_sortable_columns', '_admin_column_register_sortable' );
			add_filter('manage_wlt_campaign_posts_columns', '_admin_remove_columns');
			add_action( 'manage_wlt_campaign_posts_custom_column',  '_admin_custom_column' , 10, 3  );	
			add_filter( 'manage_wlt_campaign_posts_columns',  '_admin_custom_columns'  );	
...
function enqueue(){ global $pagenow;
	...
	if($pagenow == "post.php" || $pagenow == "post-new.php" ){ 
		...
		// JQUERY UI
		wp_register_script( 'themescripts',  FRAMREWORK_URI.'admin/js/scripts.js');
  ...
  ?>
   <input type="hidden" name="wlt_field[<?php echo $key; ?>_old]" value="<?php echo $oldIds; ?>" /> 
	...
	if(isset($val['dateitem'])){ 
			...
			<span class="add-on"><i class="gicon-calendar"></i></span>
...
<?php echo get_user_meta($user->ID,'login_ip',true); ?>
   ...
   function parse_hook($name){
	...
	function wlt_update_icon_field($term_id) {
		...
		if(strpos($_POST['_wp_original_http_referer'], THEME_TAXONOMY."_type") != false || strpos($_POST['_wp_original_http_referer'], "cproduct_type") ){
...
function _admin_menu() {
	...
	if($license != ""){
		...
		*/
		...
		add_menu_page('premiumpress_addons', "Theme Plugins", $DEFAULT_STATUS, 'premiumpress_addons', '_admin_page_10', ''.get_bloginfo('template_url').'/framework/admin/img/m10.png', '3.2'); 
...
function my_URL_menu1() {
	add_menu_page( 'My_URL_menu', 'Child Themes', 'read', 'extrathemes', ''.get_bloginfo('template_url').'/framework/admin/img/m0.png', ''.get_bloginfo('template_url').'/framework/admin/img/m0.png', '3.1' );
}
